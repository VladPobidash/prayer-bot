...
| `src/timezone.ts` | Per-user day boundaries: `isValidTimezone` (cached `Intl.DateTimeFormat` probe), `userTimezone(telegramId)` (`users.timezone`, falling back to `FALLBACK_TIMEZONE` = UTC when unknown or invalid), `userToday` and `userNowHHMM`. The zone is reported by the Mini App on every start; there is **no server-wide `TZ`**. See ADR 18. |
| `src/preferences.ts` | Committed code-reviewed tunables (`TELEGRAM_MAX_LENGTH`, `PAGE_SIZE`, `FALLBACK_TIMEZONE`) and the `LOG_PREFIX` constants used by every log call. |
...
| `src/accountability.ts` | Stage 3 accountability: pure `computeMissStreak` (wall-clock derived — streak = days between `max(lastPrayed, joinDate)` and yesterday, so join-day grace and prayer-resets are automatic) + `decideAction` (warn at ≥2 missed days once per streak; remove at ≥5 only when the warning is ≥3 days old — a warning always precedes removal, even after downtime), and `evaluateAccountability(now, notify)` — hourly sweep over plain members of active rooms, resolving "today" in **each member's own timezone** with an injected `NotifyFn`; members with no shared or other-member personal topic are exempt and have stale warnings cleared; removal reuses `rooms.leaveRoom` and DMs both the member and the room admin. Per-member try/catch: one failed DM never blocks the sweep. |
| `src/reminders.ts` | Per-topic daily dispatch with an injected `send: SendFn`. `buildMessagesForUser` gathers today's shared + personal assignments across all of a user's active rooms (generates assignments on first touch, idempotent). `dispatchDueReminders(now, send)` iterates all users with a reminder time set and resolves the date and clock **per recipient** (`users.timezone`), skips those whose own local time hasn't reached their reminder, skips those already sent today (`hasSentToday` — idempotent + catch-up safe), then sends one message per topic (or a plain daily nudge if a room has no eligible topic) and records each via `recordSent`. `streakLine(telegramId, date, locale)` builds the 🔥 streak footer, appended to the **first** message of the day only. |
...
| `src/scheduler.ts` | `register({ send, notify })` — schedules a per-minute node-cron job calling `dispatchDueReminders(new Date(), deps.send)` and an hourly job calling `evaluateAccountability(new Date(), deps.notify)`; both tick in plain UTC because every day boundary inside them is per user. Returns stoppable `ScheduledTask[]`. `SendFn` / `NotifyFn` are re-exported from `reminders.ts` / `accountability.ts`. |
