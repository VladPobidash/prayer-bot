...
test('Full Mini App E2E Flow: Static assets, Auth, Room Lifecycle, Topics, Updates, Prayer Logging, and Settings', async () => {
  initDb(':memory:');
  ...
  const roomDetail = (await roomDetailRes.json()) as { members: any[]; sharedTopics: any[]; personalTopics: any[] };
  assert.equal(roomDetail.members.length, 2);
  assert.equal(roomDetail.sharedTopics.length, 1);
  assert.equal(roomDetail.personalTopics.length, 1);
